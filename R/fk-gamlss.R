...
{
  list( degree=degree, all.fixed=all.fixed, fixed = fixed, base=base)
}                      
################################################################################
################################################################################
#----------------------------------------------------------------------------------------
...
startLambdaName <- as.character(attr(x, "NameForLambda"))  
   ...
   #   cat(lambda)
     ...
     {
      fit <- fitFixedKnots(y=y, x=xvar,  weights=w, degree=degree, 
                           knots = lambda, fixed=control$fixed,  
                           base = control$base)
     }
     else
     {
       fit <- fitFreeKnots(y=y, x=xvar,  weights=w, degree=degree, 
                           knots = lambda, fixed = control$fixed, 
                           base = control$base)     
     }
